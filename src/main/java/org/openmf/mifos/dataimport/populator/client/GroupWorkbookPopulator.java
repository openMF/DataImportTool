...
import org.openmf.mifos.dataimport.populator.CenterSheetPopulator;
...
public class GroupWorkbookPopulator extends AbstractWorkbookPopulator {
	...
	public GroupWorkbookPopulator(OfficeSheetPopulator officeSheetPopulator, PersonnelSheetPopulator personnelSheetPopulator,CenterSheetPopulator centerSheetPopulator,
    ...
    public Result populate(Workbook workbook) {
    	...
    	if(result.isSuccess())
     	   result = centerSheetPopulator.populate(workbook);
    	if(result.isSuccess())
    ...
    private void setNames(Sheet worksheet, ArrayList<String> officeNames) {
    	...
    	//Client,Staff and Center  Names for each office
    	for(Integer i = 0; i < officeNames.size(); i++) {
    		...
    		Integer[] officeNameToBeginEndIndexesOfCenter = centerSheetPopulator.getOfficeNameToBeginEndIndexesOfCenters().get(i);
    		Name clientName = groupWorkbook.createName();
    		Name loanOfficerName = groupWorkbook.createName();
    		Name centerName= groupWorkbook.createName();
			...
			if(officeNameToBeginEndIndexesOfCenter != null) {
     	        centerName.setNameName("Center_" + officeNames.get(i));
     	        centerName.setRefersToFormula("Center!$B$" + officeNameToBeginEndIndexesOfCenter[0] + ":$B$" + officeNameToBeginEndIndexesOfCenter[1]);
     		 }
    	}
    }
