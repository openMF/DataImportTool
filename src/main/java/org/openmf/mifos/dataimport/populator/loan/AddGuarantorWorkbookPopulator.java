...
public class AddGuarantorWorkbookPopulator extends AbstractWorkbookPopulator {
	...
	private Result parseLoans() {
	 ...
	 @Override
	    public Result populate(Workbook workbook) {
	        ...
	        return setRules(addGuarantorSheet);
	        if(result.isSuccess()) 
	    		result = populateLoansTable(addGuarantorSheet);
	        if(result.isSuccess()) 
	...
	private void setNames(Sheet worksheet) {
    	...
    	for(int j = 0; j < clientsWithActiveLoans.size(); j++) {
    		...
    		name.setRefersToFormula("guarantor!$Q$" + clientNameToBeginEndIndexes.get(clientsWithActiveLoans.get(j))[0] + ":$Q$" + clientNameToBeginEndIndexes.get(clientsWithActiveLoans.get(j))[1]);
    	}
	}
	...
	private void setLayout(Sheet worksheet) {
		Row rowHeader = worksheet.createRow(0);
		 worksheet.setColumnWidth(OFFICE_NAME_COL, 4000);
	        ...
	        worksheet.setColumnWidth(SAVINGS_ID, 3000);
	        ...
	        writeString(SAVINGS_ID, rowHeader, "Savings Account Id");
